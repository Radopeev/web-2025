    ...
    <section>
        ...
        <?php elseif (isset($project) && $project): ?>
            <div>
                <h2>
                    Project Information
                    <a href="/project/edit?id=<?php echo htmlspecialchars($project['id']); ?>"
                        style="margin-left: 15px; padding: 5px 10px; background-color: #007bff; color: white; border-radius: 5px; text-decoration: none;">Edit
                        Project</a>
                </h2>
                <p><strong>Description:</strong> <?php echo htmlspecialchars($project['description']); ?></p>
                <p><strong>Created At:</strong>
                    <?php echo htmlspecialchars(date('F j, Y, g:i a', strtotime($project['created_at']))); ?></p>
                ...
                <?php if (isset($project['config_file']) && $project['config_file']): ?>
                    <p>
                        ...
                        <?php if ($configIsViewableInline && $configViewType === 'text'): ?>
                            ...
                            (<a href="<?php echo htmlspecialchars($configBasePath . $configFileName); ?>"
                                download="<?php echo $configFileName; ?>">Download</a>)
                        <div style="border: 1px solid #ccc; padding: 10px; margin-top: 5px;">
                            <pre
                                style="white-space: pre-wrap; word-wrap: break-word;"><?php echo htmlspecialchars($configViewableContent); ?></pre>
                        </div>
                    <?php else: ?>
                        ...
                        <?php if ($configIsViewableInline): // If it's technically viewable but not text, offer toggle ?>
                            <a href="javascript:void(0);" onclick="toggleVisibility('<?php echo $configElementId; ?>');">
                                (View/Hide)</a>
                            <div id="<?php echo $configElementId; ?>"
                                style="display:none; border: 1px solid #ccc; padding: 10px; margin-top: 5px;">
                                <?php if ($configViewType === 'image'): ?>
                                    <img src="<?php echo htmlspecialchars($configBasePath . $configFileName); ?>"
                                        alt="<?php echo $configFileName; ?>" style="max-width: 100%; height: auto;">
                                <?php elseif ($configViewType === 'pdf'): ?>
                                    <iframe src="<?php echo htmlspecialchars($configBasePath . $configFileName); ?>"
                            ...
                            <li>
                                <?php if (!empty($filePath)): ?>
                                    <?php if ($isViewableInline && $viewType === 'text'): ?>
                                        ...
                                        (<a href="<?php echo htmlspecialchars($sourceBasePath . $filePath); ?>"
                                            download="<?php echo htmlspecialchars($fileNameToDisplay); ?>">Download</a>)
                                        <div style="border: 1px solid #ccc; padding: 10px; margin-top: 5px;">
                                            <pre
                                                style="white-space: pre-wrap; word-wrap: break-word;"><?php echo htmlspecialchars($viewableContent); ?></pre>
                                        </div>
                                    <?php else: ?>
                                        ...
                                        <?php if ($isViewableInline): // If it's image/pdf, offer toggle ?>
                                            <a href="javascript:void(0);" onclick="toggleVisibility('<?php echo $elementId; ?>');">
                                                (View/Hide)</a>
                                            <div id="<?php echo $elementId; ?>"
                                                style="display:none; border: 1px solid #ccc; padding: 10px; margin-top: 5px;">
                                                <?php if ($viewType === 'image'): ?>
                                                    <img src="<?php echo htmlspecialchars($sourceBasePath . $filePath); ?>"
                                                        alt="<?php echo htmlspecialchars($fileNameToDisplay); ?>"
                                                        style="max-width: 100%; height: auto;">
                                                <?php elseif ($viewType === 'pdf'): ?>
                                                    <iframe src="<?php echo htmlspecialchars($sourceBasePath . $filePath); ?>"
                                                        style="width:100%; height:600px; border:none;"></iframe>
                                                <?php else: ?>
                                                    <p>This file type can be viewed inline but content not prepared. Please download.</p>
                                                <?php endif; ?>
            ...
            <?php if (isset($selectedFileContent) && $selectedFileContent !== null): ?>
                <div id="file-content-viewer">
                    ...
                    if (strpos($selectedFileMimeType, 'text/') === 0 || $selectedFileMimeType === 'application/json' || $selectedFileMimeType === 'application/xml'):
                        ?>
                        <pre
                            style="white-space: pre-wrap; word-wrap: break-word; background-color: #f0f0f0; padding: 10px; border: 1px solid #ccc; max-height: 400px; overflow-y: auto;"><?php echo htmlspecialchars($selectedFileContent); ?></pre>
                    <?php elseif (strpos($selectedFileMimeType, 'image/') === 0): ?>
                        <img src="data:<?php echo htmlspecialchars($selectedFileMimeType); ?>;base64,<?php echo base64_encode($selectedFileContent); ?>"
                            alt="<?php echo htmlspecialchars($selectedFileName); ?>"
                            style="max-width: 100%; height: auto; display: block; margin: 0 auto;">
                    <?php elseif ($selectedFileMimeType === 'application/pdf'): ?>
                        <p>PDF files are best viewed in a new tab or downloaded. Please use the Download link or implement a
                            dedicated PDF viewer.</p>
                    <?php else: ?>
                ...
                <?php if (!empty($projectInstruments)): ?>
                    <ul>
                        <?php foreach ($projectInstruments as $instrument): ?>
                            ...
                            ?>
                            <li>
                                <strong><?php echo htmlspecialchars($instrument['name'] ?? 'Unknown'); ?></strong> (Type:
                                <?php echo htmlspecialchars($instrument['type'] ?? 'N/A'); ?>)
                                <p><?php echo htmlspecialchars($instrument['description'] ?? ''); ?></p>
                                <?php if (!empty($instrument['access_link'])): ?>
                                    <p>Access: <a href="<?php echo htmlspecialchars($instrument['access_link']); ?>" target="_blank"
                                            rel="noopener noreferrer"><?php echo htmlspecialchars($instrument['access_link']); ?></a>
                                    </p>
                                <?php endif; ?>
                            </li>
                        <?php endforeach; ?>
